...
bool StringAttributeCollector::startSection(std::string const& name) { return true; }
void StringAttributeCollector::endSection() {}

void StringAttributeCollector::operator()(
	std::string const& name, bool  b, bool is_default)
{
	(*this)(name, b ? "true" : "false", is_default);
}
void StringAttributeCollector::operator()(
	std::string const& name, float f, bool is_default)
{
	(*this)(name, to_string(f), is_default);
}
void StringAttributeCollector::operator()(
	std::string const& name, float x, float y, bool is_default)
{
	(*this)(name, to_string(x) + " " + to_string(y), is_default);
}
void StringAttributeCollector::operator()(
	std::string const& name, float x, float y, float w, float h, bool is_default)
{
	(*this)(name, to_string(x) + " " + to_string(y) + " " + to_string(w) + " " + to_string(h), is_default);
}
void StringAttributeCollector::operator()(
	std::string const& name, Color const& c, bool is_default)
{
	if(c.a != 1) {
...
void StringAttributeCollector::operator()(
	std::string const& name, const char* s, bool is_default)
{
